...
def plot_churn_ads(features, path):
    fig, ax = plt.subplots(figsize=(8,5))
    report_bins = pd.cut(features['ads_watched_7d'], bins=5)
    churn_by_reports = features.groupby(report_bins, observed=True)['churn_d7'].mean()
    churn_by_reports.plot(kind='bar', ax=ax, color='#22d3ee', edgecolor='#8b5cf6')
    ax.set_title('Churn D7 vs rewarded ads vistos (7d)', color='#22d3ee', fontsize=14)
...
def plot_winrate_churn(features, path):
    ...
    ax.set_title('Engagement (monedas/sesion) por churn', color='#22d3ee', fontsize=14)
    ax.set_xlabel('Monedas por sesion (proxy)', color='#a0a0c0')
...
def main():
    ...
    plot_churn_ads(features, f"{IMG_DIR}/churn_toxicidad.png")
